...
adni_output_dir = '/ADNI/CLASSIFICATION/OUTPUTS_GENERALIZATION'
output_dir = '/AIBL/CLASSIFICATION/OUTPUTS_GENERALIZATION'
...
for image_type in image_types:
    for fwhm in smoothing:
        for task in tasks:
            ...
            adni_subjects_visits_tsv = path.join(adni_tasks_dir, '%s_vs_%s_subjects_sessions_generalization_49.tsv' % (task[0], task[1]))
            adni_diagnoses_tsv = path.join(adni_tasks_dir, '%s_vs_%s_diagnoses_generalization_49.tsv' % (task[0], task[1]))
            ...
            adni_classifier_dir = path.join(adni_output_dir, image_type, 'voxel_based',
                                                'smooothing-%s' % fwhm, 'linear_svm',
                                                '%s_vs_%s_49' % (task[0], task[1]), 'classifier')
            ...
            adni_images = CAPSVoxelBasedInput(adni_caps_dir, adni_subjects_visits_tsv, adni_diagnoses_tsv, group_id,
                                              image_type, fwhm, modulated='on', pvc=None, mask_zeros=False)
            ...
            iteration_subjects_df.to_csv(path.join(classification_dir, 'subjects_voxel.tsv'),
