...
caps_dir = '/ADNI/CAPS/'
output_dir = '/ADNI/CLASSIFICATION/OUTPUTS_GENERALIZATION'
...
for image_type in image_types:
    for fwhm in smoothing:
        ...
        for task in tasks:
            ...
            subjects_visits_tsv = path.join(tasks_dir, '%s_vs_%s_subjects_sessions_generalization_49.tsv' % (task[0], task[1]))
            diagnoses_tsv = path.join(tasks_dir, '%s_vs_%s_diagnoses_generalization_49.tsv' % (task[0], task[1]))

            classification_dir = path.join(output_dir, image_type, 'voxel_based',
                                           'smooothing-%s' % fwhm, 'linear_svm',
                                           '%s_vs_%s_49' % (task[0], task[1]))
            if not path.exists(classification_dir):
                os.makedirs(classification_dir)
...
for image_type in image_types:
    for atlas in atlases:
        for task in tasks:
            ...
            subjects_visits_tsv = path.join(tasks_dir,
                                            '%s_vs_%s_subjects_sessions_generalization_49.tsv' % (task[0], task[1]))
            diagnoses_tsv = path.join(tasks_dir, '%s_vs_%s_diagnoses_generalization_49.tsv' % (task[0], task[1]))
