...
output_dir = '/DATASETS/OUTPUTS/LEARNING_CURVE_ADNI_AIBL_OASIS'

group_id = 'ADNIbl'
image_types = ['T1']
tasks_dir = '/DATASETS/SUBJECTS/LEARNING_CURVE_ADNI_AIBL_OASIS'
tasks = [('CN', 'AD')]
...
for image_type in image_types:
    for fwhm in smoothing:
        for task in tasks:
            diagnoses_caps_tsv = path.join(tasks_dir, '%s_vs_%s_diagnoses_balanced_caps.tsv' % (task[0], task[1]))
