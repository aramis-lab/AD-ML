...
from clinica.pipelines.machine_learning import base, input
...
class MultipleCAPSInput(base.MLInput):

    def __init__(self, caps_directory, diagnoses_caps_tsv, group_id, image_type, precomputed_kernel=None):
        """
        ...
        diagnoses = parsers.read_csv(diagnoses_caps_tsv, sep='\t')

        #TODO correct verification
        # if list(diagnoses.columns.values) != ['participant_id', 'session_id', 'diagnosis', 'caps']:
        #     raise Exception('Subjects, visits, diagnoses, CAPS file is not in the correct format.')
        ...
        self._caps = list(diagnoses.caps)
...
class MultipleCAPSVoxelBasedInput(MultipleCAPSInput):

    def __init__(self, caps_directory, diagnoses_caps_tsv, group_id, image_type, fwhm=0,
        ...
        super(MultipleCAPSVoxelBasedInput, self).__init__(caps_directory, diagnoses_caps_tsv, group_id, image_type,
    ...
    def get_images(self):
        ...
        if self._image_type == 'T1':
            ...
            self._images = [path.join(self._caps_directory, 'CAPS_' + self._caps[i], 'subjects', self._subjects[i], self._sessions[i],
...
class MultipleCAPSRegionBasedInput(MultipleCAPSInput):
    ...
    def __init__(self, caps_directory, diagnoses_caps_tsv, group_id, image_type, atlas,
        ...
        super(MultipleCAPSRegionBasedInput, self).__init__(caps_directory, diagnoses_caps_tsv, group_id, image_type,
                                                           precomputed_kernel=precomputed_kernel)
